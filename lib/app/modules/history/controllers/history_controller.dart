...
class OrderHistoryController extends GetxController {
  ...
  Future<void> addOrderHistory(OrderHistory order) async {
    try {
      await firestore.collection('orderHistory').doc(order.orderId).set(order.toMap());
    } catch (e) {
      print('Error adding order history: $e');
    }
  }

  Future<List<OrderHistory>> loadOrderHistory(String userId) async {
    try {
      QuerySnapshot querySnapshot = await firestore.collection('orderHistory')
          .where('userId', isEqualTo: userId)
          .orderBy('orderDate', descending: true)
          .get();
      
      return querySnapshot.docs.map((doc) => OrderHistory.fromMap(doc.data() as Map<String, dynamic>)).toList();
    } catch (e) {
